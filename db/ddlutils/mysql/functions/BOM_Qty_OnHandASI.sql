 ...
 * 
 * Title.........: 
 * Description...: Return quantity on hand for BOM ASI
 *                                        Prd, ASI, Wrh, Lct
 * Test..........: SELECT BomQtyOnHandASI(123,   0, 103, 101)$$   => 20.0000000000
 *                 SELECT BomQtyOnHandASI(123, null, 103, 101)$$  => 20.0000000000
 *                 SELECT BomQtyOnHandASI(133,   0, 103, null)$$  => 30.0000000000
 *                 SELECT BomQtyOnHandASI(133,   0, null, 101)$$  => 30.0000000000
 *                 SELECT BomQtyOnHandASI(133,   0, null, null)$$ =>  0.0000000000
 *                 SELECT BomqtyonhandASI(  0,   0, null, 101)$$  =>  0.0000000000
 *
 *                 SET @result = 0.0;
 *                 Call BomQtyOnHandASIProc(133, 0, 103, 101, @result)  => 
 *                 SELECT @result;
 *                 +---------------+
 *                 | @result       |
 *                 +---------------+
 ...
 *                 +---------------+
 *
 *                 M_Product_ID |  Name | IsBOM | ProductType | IsStocked | InstanceAttribute
 *                          123 |   Oak |     N |           I | Y         | 
 *                          124 |   Elm |     N |           I | Y         | 
 *                          133 | PChair|     Y |           I | Y         | Y
 *                 M_Warehouse
...
-- ## Drop statement
DROP PROCEDURE IF EXISTS BomQtyOnHandASIProc $$

-- ## Create statement
CREATE PROCEDURE BomQtyOnHandASIProc (
 ...
 OUT  BomQtyOnHandASI          DECIMAL(20, 10)
)
  DETERMINISTIC
proc: BEGIN
  ...
  ELSEIF (IsStocked='Y') THEN
    ...
    --
    SET BomQtyOnHandASI = ProductQty;
    LEAVE proc;
  END IF;
  ...
  IF (Quantity > 0) THEN
    ...
    --
    SET BomQtyOnHandASI = ROUND (Quantity, StdPrecision);
      LEAVE PROC;
  END IF;
  SET BomQtyOnHandASI = 0;
END$$
...
DROP FUNCTION IF EXISTS BomQtyOnHandASI
$$

CREATE FUNCTION BomQtyOnHandASI (
  Product_ID               DECIMAL(10, 0),
  AttributeSetInstance_ID  DECIMAL(10, 0),
  Warehouse_ID             DECIMAL(10, 0),
  Locator_ID               DECIMAL(10, 0)  -- Only used, if warehouse is null
...
BEGIN
  DECLARE BomQtyOnHandASI DECIMAL(20, 10);
  CALL BomQtyOnHandASIProc (
    Product_ID,
    AttributeSetInstance_ID,
    Warehouse_ID,
