 ...
 * 
 * Title.........: 
 * Description...: Return quantity Reserved for BOM ASI
 *                                          Prd, ASI, Wrh, Lct
 * Test..........: SELECT BomQtyReservedASI(123,   0, 103, 101)$$   => 0.0000000000
 *                 SELECT BomQtyReservedASI(133,   0, 103, null)$$  => 0.0000000000
 *                  SELECT BomQtyReservedASI(133, 1000013, 103, null)$$  => 0.0000000000
 *                 SELECT BomQtyReservedASI(133,   0, null, 101)$$  => 0.0000000000
 *                 SELECT BomQtyReservedASI(133,   0, null, null)$$ => 0.0000000000
 *                 SELECT BomQtyReservedASI(  0,   0, null, 101)$$  => 0.0000000000
 * 
 *                 SET @result = 0.0;
 *                 Call BomQtyReservedASIProc(123, 0, 103, 101, @result)  => 
 *                 SELECT @result;
 *                 +---------------+
 *                 | @result       |
 *                 +---------------+
...
-- ## Drop statement
DROP PROCEDURE IF EXISTS BomQtyReservedASIProc $$

-- ## Create statement
CREATE PROCEDURE BomQtyReservedASIProc (
  ...
  Out  BomQtyReservedASI        DECIMAL(20, 10)
)
  DETERMINISTIC
proc: BEGIN
  DECLARE myWarehouse_ID DECIMAL(10, 0);
  DECLARE Quantity DECIMAL(20, 10) DEFAULT 12345.67; -- unlimited [99999]
  ...
  ELSEIF (IsStocked='Y') THEN
    ...
    --
    SET BomQtyReservedASI = ProductQty;
    LEAVE proc;
  END IF;
  ...
  IF (Quantity > 0) THEN
    ...
    --
    SET BomQtyReservedASI = ROUND (Quantity, StdPrecision);
    LEAVE proc;
  END IF;
...
DROP FUNCTION IF EXISTS BomQtyReservedASI
$$

CREATE FUNCTION BomQtyReservedASI (
  p_Product_ID             DECIMAL(10, 0),
  p_AttributeSetInstance_ID  DECIMAL(10, 0),
  p_Warehouse_ID           DECIMAL(10, 0),
  p_Locator_ID             DECIMAL(10, 0)  -- Only used, if warehouse is null
)
