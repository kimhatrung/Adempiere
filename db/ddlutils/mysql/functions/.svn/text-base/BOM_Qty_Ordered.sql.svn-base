 ...
 *                                     Prd, Wrh, Lct
 * Test..........: SELECT bomQtyOrdered(123, 103, 101)$$   => 0.0000000000
 *                 SELECT bomQtyOrdered(133, 103, null)$$  => 0.0000000000
 *                 SELECT bomQtyOrdered(133, null, 101)$$  => 0.0000000000
 *                 SELECT bomQtyOrdered(133, null, null)$$ => 0.0000000000
 *                 SELECT bomQtyOrdered(0, null, 101)$$    => 0.0000000000
 * 
 *                 SET @result = 0.0;
 *                 Call bomQtyOrderedProc(123, 103, 101, @result)  => 
 *                 SELECT @result;
 *                 +---------------+
 *                 | @result       |
 *                 +---------------+
...
-- ## Drop statement
DROP PROCEDURE IF EXISTS bomQtyOrderedProc $$

-- ## Create statement
CREATE PROCEDURE bomQtyOrderedProc (
      Product_ID     DECIMAL(10, 0),
      Warehouse_ID   DECIMAL(10, 0),
      Locator_ID     DECIMAL(10, 0),  -- Only used, if warehouse is null
 OUT  BomQtyOrdered  DECIMAL(20, 10)
)
  DETERMINISTIC
proc: BEGIN
  ...
  myLoop: LOOP
	...
	IF (Quantity > 0) THEN
      -- Get Rounding Precision for Product
		...
		--
		SET BomQtyOrdered = ROUND (Quantity, StdPrecision);
        LEAVE proc;
	END IF;
	SET BomQtyOrdered = 0;
END$$
...
DROP FUNCTION IF EXISTS bomQtyOrdered $$

CREATE FUNCTION bomQtyOrdered (
...
BEGIN
  DECLARE v_BomQtyOrdered DECIMAL(20, 10);
  CALL BomQtyOrderedProc (
    p_Product_ID,
    p_Warehouse_ID,
    p_Locator_ID,
