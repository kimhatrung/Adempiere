 ...
 * 
 * Title.........: 
 * Description...: Return quantity Reserved for BOM
 *                                     Prd, Wrh, Lct
 * Test..........: SELECT BomQtyReserved(123, 103, 101)$$   => 0.0000000000
 *                 SELECT BomQtyReserved(133, 103, null)$$  => 0.0000000000
 *                 SELECT BomQtyReserved(133, null, 101)$$  => 0.0000000000
 *                 SELECT BomQtyReserved(133, null, null)$$ => 0.0000000000
 *                 SELECT BomQtyReserved(0, null, 101)$$    => 0.0000000000
 * 
 *                 SET @result = 0.0;
 *                 Call BomQtyReservedProc(123, 103, 101, @result)  => 
 *                 SELECT @result;
 *                 +---------------+
 *                 | @result       |
 *                 +---------------+
...
-- ## Drop statement
DROP PROCEDURE IF EXISTS BomQtyReservedProc $$

-- ## Create statement
CREATE PROCEDURE BomQtyReservedProc (
      Product_ID      DECIMAL(10, 0),
      Warehouse_ID    DECIMAL(10, 0),
      Locator_ID      DECIMAL(10, 0),  -- Only used, if warehouse is null
  OUT BomQtyReserved  DECIMAL(20, 10)
)
  DETERMINISTIC
proc: BEGIN
  ...
  myLoop: LOOP
		...
		IF (bom_ProductType = 'I' AND bom_IsStocked = 'Y') THEN
			-- Get ProductQty
			SELECT COALESCE(SUM(s.QtyReserved), 0)
			  INTO ProductQty
			FROM   M_STORAGE s
  ...
  IF (Quantity > 0) THEN
    ...
    --
    SET BomQtyReserved = ROUND (Quantity, StdPrecision);
    LEAVE proc;
  END IF;
  SET BomQtyReserved = 0;
END$$
...
DROP FUNCTION IF EXISTS BomQtyReserved
$$

CREATE FUNCTION BomQtyReserved(
